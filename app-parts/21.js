      ...
      // Eye-level labels: migrate the old 18/30/35 px defaults to 40 px and remove the
      ...
      ensureEyeLabelSettings=function(){
        const existing=project.settings?.eyeLevelLabels||{},legacyDefault=(existing.sizePx===18&&(existing.maxVisible===16||existing.maxVisible===undefined)&&!existing.default30Migrated)||(existing.sizePx===30&&existing.default30Migrated&&!existing.default35Migrated)||(existing.sizePx===35&&existing.default35Migrated&&!existing.default40Migrated);
        const hadSize=Number.isFinite(+existing.sizePx),settings=ensureEyeLabelSettingsBeforeV29();
        if(!hadSize||legacyDefault)settings.sizePx=40;
        settings.sizePx=Math.max(12,Math.min(50,+settings.sizePx||40));settings.maxVisible=Number.MAX_SAFE_INTEGER;settings.default30Migrated=true;settings.default35Migrated=true;settings.default40Migrated=true;return settings;
      };
      const maxVisibleFieldV29=$('eyeLabelMax');if(maxVisibleFieldV29)maxVisibleFieldV29.closest('label')?.remove();
      const labelSizeFieldV29=$('eyeLabelSize');if(labelSizeFieldV29){labelSizeFieldV29.value='40';labelSizeFieldV29.max='50';}
