      ...
      async function exportProjectPackage(){
        ...
        try{
          const zip=new JSZip(),portable=JSON.parse(JSON.stringify(project)),manifest={format:PROJECT_PACKAGE_FORMAT,formatVersion:2,appVersion:APP_VERSION,name:portable.meta?.name||'Untitled layout',createdAt:new Date().toISOString(),projectFile:'project.json',notesFile:'project-notes.json',designFile:'design/design-spec.json',renderSpecDirectory:'render-specs/',workflow:portable.workflow||null,basemap:null,references:[]};
          ...
          zip.file('manifest.json',JSON.stringify(manifest,null,2));zip.file('project.json',JSON.stringify(portable,null,2));zip.file('project-notes.json',JSON.stringify(portable.meta||{},null,2));zip.file('design/design-spec.json',JSON.stringify(portable.design||{},null,2));
          (portable.design?.shotRenderSpecs||[]).forEach(spec=>zip.file(`render-specs/${safeFileName(spec.shotId||spec.id,'shot')}.json`,JSON.stringify(spec,null,2)));
          ...
          downloadBlob(blob,projectDownloadName('zip'));setPackageStatus('Approved Step 2 ZIP exported with project JSON, design metadata, locked camera shots, render specs and project assets.','ok');
