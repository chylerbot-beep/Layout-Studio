      ...
      updateWallDrag=function(event){
        ...
        const projected={x:fixed.x+ux*scalar,y:fixed.y+uy*scalar},target=event.altKey?projected:magneticWallPointV45(projected,wall.id,{x:fixed.x,y:fixed.y,ux,uy});
        const x1=movingStart?target.x:fixed.x,y1=movingStart?target.y:fixed.y,x2=movingStart?fixed.x:target.x,y2=movingStart?fixed.y:target.y;
        setWallFromEndpoints(wall,x1,y1,x2,y2,original.thickness,false);
        const now=performance.now();if(now-wallDragLastRender>32){wallDragLastRender=now;refreshArchitectureVisuals(false);populateArchitectureFields('wall',wall);$('toolHint').textContent=target.snapped?'Ctrl constraint active: endpoint magnetically joined on the original wall line.':'Ctrl constraint active: wall length is changing on its original line.';}
      };
      ...
      function physicalOpeningShapesV45(){const shapes=[];(project.openings||[]).forEach(opening=>{const wall=(project.walls||[]).find(item=>item.id===opening.wallId);if(!wall)return;const e=wallMetrics(wall),from=Math.max(0,(+opening.offset||0)-(+opening.width||0)/2),to=Math.min(e.length,(+opening.offset||0)+(+opening.width||0)/2);shapes.push({...wallSegmentShapeV28(wall,from,to,+opening.sill||0,(+opening.sill||0)+(+opening.height||0)),opening});});return shapes;}
      ...
      validate=function(){
        clearGroup(validationOverlayGroupV28);const enabled=ensurePhysicalValidationSettingsV28(),warnings=[],problemFurniture=new Set(),problemWalls=[],problemOpenings=new Set(),problemShell=[];
        ...
        if(enabled){
          ...
          const openingShapes=physicalOpeningShapesV45();items.forEach(entry=>openingShapes.forEach(shape=>{if(shapesOverlapV28(entry.shape,shape)){warnings.push(`${entry.item.name} overlaps ${shape.opening.name}`);problemFurniture.add(entry.item.id);problemOpenings.add(shape.opening.id);}}));
          items.forEach(entry=>wallMountedOpeningIssuesV45(entry.item,{x:entry.item.x,y:entry.item.y,rotation:entry.item.rotation||0}).forEach(opening=>{warnings.push(`${entry.item.name} is mounted across ${opening.name}`);problemFurniture.add(entry.item.id);problemOpenings.add(opening.id);}));
          const shellItems=physicalShellItemsV42();items.forEach(entry=>shellItems.forEach(shell=>{const shape=itemShapeV28({...shell,elevation:shell.elevation||0,rotation:shell.rotation||0});if(shapesOverlapV28(entry.shape,shape)){warnings.push(`${entry.item.name} intersects ${shell.name}`);problemFurniture.add(entry.item.id);if(!problemShell.includes(shell))problemShell.push(shell);}}));
          problemFurniture.forEach(id=>{const object=furnitureGroup.children.find(mesh=>mesh.userData.id===id);if(object)addValidationObjectOutlineV28(object);});problemWalls.forEach(addValidationWallOutlineV28);problemOpenings.forEach(id=>{const object=openingGroup.children.find(mesh=>mesh.userData.id===id);if(object)addValidationObjectOutlineV28(object);});problemShell.forEach(addValidationShellOutlineV28);
        }
