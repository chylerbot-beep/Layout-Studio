      ...
      if(furnitureAlignRow&&$('alignAllWalls'))furnitureAlignRow.appendChild($('alignAllWalls'));
      if(furnitureAlignRow&&!$('alignFurniture')){
        const button=document.createElement('button');button.id='alignFurniture';button.textContent='Align furniture β';
        button.title='Conservatively repair furniture that intersects walls, leaves the plan, blocks clearances, or should remain anchored to a wall. One undo restores the previous layout.';
      ...
      if(!$('furnitureAlignStatus')&&$('basemapStatus')){
        ...
        status.textContent='Align furniture β preserves room, wall, support and dining-group intent. It only moves objects when a placement is invalid or clearly wall-anchored.';
      ...
      function nearestWallForFurniture(item,walls,preferredId=null){
        ...
        let best=null;(walls||[]).forEach(wall=>{const data=wallDistanceForItem(item,wall);if(!best||data.surfaceDistance<best.surfaceDistance)best={wall,...data};});return best;
      }
      ...
      function wallAlignedCandidate(item,wall,room,sideSign=null){
        ...
        const halfNormal=itemProjectionHalf(item,rotation,nx,ny),halfTangent=itemProjectionHalf(item,rotation,e.ux,e.uy),gap=Math.max(0,+item.placement?.gap||20),desired=sign*(e.thickness/2+halfNormal+gap);
        ...
        return{x:Math.round((cx-item.w/2)/25)*25,y:Math.round((cy-item.d/2)/25)*25,rotation:Math.round(rotation*10)/10,wallId:wall.id};
      }
      ...
      function candidateIssues(item,candidate,items,excludeIds,context){
        ...
        context.walls.forEach(wall=>{if(spansOverlap(vertical,{min:0,max:wall.h||2700})&&boxesOverlap2D(box,wallObstacleBox(wall)))issues.push('wall');});
      ...
      function alignSingleFurniture(item,items,context){
        if((+item.elevation||0)>25||item.category==='decorative')return{moved:false,rotated:false,unresolved:false};
        ...
        const bases=[original];if(shouldSnap){bases.push(wallAlignedCandidate(item,nearest.wall,room,1),wallAlignedCandidate(item,nearest.wall,room,-1));}
        ...
        bases.forEach(base=>{const maxRadius=base===original?1200:600;furnitureAlignOffsets.forEach(offset=>{if(offset.d>maxRadius)return;const candidate={x:base.x+offset.dx,y:base.y+offset.dy,rotation:base.rotation};const result=candidateScore(item,candidate,original,room,items,new Set([item.id]),context,shouldSnap?nearest.wall:null);if(result.score<best.score)best={candidate,...result};});});
        ...
        if(shouldSnap)item.placement={...(item.placement||{}),mode:'wall',wallId:nearest.wall.id,gap:Math.max(0,+item.placement?.gap||20)};else item.placement={...(item.placement||{}),mode:item.placement?.mode||'free'};
